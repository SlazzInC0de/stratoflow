...
Validation rules are sensor-type-aware - a temperature sensor and a pressure sensor have different valid ranges, so the schema checks each row against the correct bounds for its sensor_type, not a single blanket range.
...
- Partition Parquet output by date/sensor_type for faster querying on large datasets
...
- Move from use_cli local Terraform auth to OIDC federated credentials for CI/CD, if tenant restrictions were lifted
